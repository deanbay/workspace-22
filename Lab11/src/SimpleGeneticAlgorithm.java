...
public class SimpleGeneticAlgorithm
{
	...
	public ScalesChrome RunSGA(boolean report)
	{
		InitPop();
	...
	private void InitPop()
	{
		...
		{
			population.add(new ScalesChrome(nbits));
		}
	}
	...
	private void DoOnePointCrossOver(int p1id,int p2id)
	{
		...
		int xopos = CS2004lab8.UI(1,nbits-1);
	...
	private void CrossOver()
	{
		...
		for(int i=0;i<popsize;++i)
		{
			if (CS2004lab8.UR(0.0,1.0) < crate) xlist.add(i);
		}
		...
		while(xlist.size() > 1)
		{
			...
			while(a >= b)
			{
				a = CS2004lab8.UI(0,xlist.size()-1);
				b = CS2004lab8.UI(0,xlist.size()-1);
			}
	...
	private void Mutation()
	{
		...
		for(int i=0;i<n;++i)
		{
			...
			{
				if (CS2004lab8.UR(0.0,1.0) < mrate)
				{
	...
	{
		Collections.sort(population,new CompareScalesChrome());
